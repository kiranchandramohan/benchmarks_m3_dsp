...
void initialize(int* restrict image_buffer1, int* restrict image_buffer2, 
		int* restrict image_buffer3, int start_indx, int end_indx)
{
  int i, j ;
...
/* This function convolves the input image by the kernel and stores the result
   in the output image. */
void convolve2d(int* restrict input_image, int* restrict kernel, 
	int* restrict output_image, int start_indx, int end_indx)
{
  int i;
  int j;
  int c;
...
void set_filter(int* restrict filter, int type)
{
        if(type == GAUSSIAN) {
                filter[0*K+0] = 1;
                filter[0*K+1] = 2;
...
void apply_threshold(int* restrict input_image1, int* restrict input_image2, 
	int* restrict output_image, int start_indx, int end_indx)
{
